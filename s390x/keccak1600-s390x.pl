...
# February 2019.
#
# Add kimd-assisted absort and squeeze subroutines. This is done for
# self-consistency with other Keccak-1600 modules. For reference, z14
# spends record-breaking ~0.8 cycles per processed byte [out of long
# message] in SHA3-256...
...
.globl	SHA3_absorb_kimd
.type	SHA3_absorb_kimd,\@function
.align	32
SHA3_absorb_kimd:
	stm${g}	%r6,%r7,6*$SIZE_T($sp)

	llgfr	$bsz,$bsz		# yes, even in 64-bit mode
	larl	%r6,sha3_kimd_fc
	srlg	%r7,$bsz,4
	 lgr	%r1,$A_flat		# reassign A_flat
	aghi	%r7,-4			# bsz/16-4
	 lgr	%r2,$inp		# reassign inp
	llgc	%r0,0(%r6,%r7)		# load function code

	lghi	%r6,0
	lgr	%r7,$len
	 lgr	%r3,$len		# reassign len
	dl${g}r	%r6,$bsz		# %r6 is remainder

	sl${g}r	%r3,%r6			# len - remainder
	.long	0xb93e0002		# kimd %r0,%r2
	brc	1,.-4			# pay attention to "partial completion"

	l${g}r	%r2,%r6			# return value, len % bsz

	lm${g}	%r6,%r7,6*$SIZE_T($sp)
	br	%r14
.size	SHA3_absorb_kimd,.-SHA3_absorb_kimd
___
}
...
.globl	SHA3_squeeze_kimd
.type	SHA3_squeeze_kimd,\@function
.align	32
SHA3_squeeze_kimd:
	cl${g}r	$len,$bsz
	jh	.Lxof_kimd
...
.Ltail_xof:
	a${g}hi	$len,-1			# mvc encoding
	brasl	%r1,1f
	mvc	0(1,$out),0($A_flat)
1:	ex	$len,0(%r1)
	br	%r14
...
.Lxof_kimd:
	...
	larl	%r6,sha3_kimd_fc
	srlg	%r7,$bsz,4
	 lgr	%r1,$A_flat		# copy A_flat
	aghi	%r7,-4			# bsz/16-4
	sl${g}r	$len,$bsz		# bias len
	llgc	%r0,0(%r6,%r7)		# load function code

.Loop_xof:
	a${g}hi	$bsz,-1			# mvc encoding
	brasl	%r6,2f
	mvc	0(1,$out),0($A_flat)
2:	ex	$bsz,0(%r6)
	la	$out,1($out,$bsz)
	...
	al${g}r	$len,$bsz		# unbias len
	jnz	.Ltail_xof

	br	%r14
.size	SHA3_squeeze_kimd,.-SHA3_squeeze_kimd
___
}
$code.=<<___;
.align	256
...
.type	sha3_kimd_fc,\@object
sha3_kimd_fc:
	.byte	35,-1,34,-1,33,32,36,-1

.asciz	"Keccak-1600 absorb and squeeze for s390x, CRYPTOGAMS by \@dot-asm"
___

# unlike 32-bit shift 64-bit one takes three arguments
$code =~ s/(srlg\s+)(%r[0-9]+),\s*([0-9]+)/$1$2,$2,$3/gm;
