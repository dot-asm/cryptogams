...
.type	poly1305_blocks,\@function
poly1305_blocks:
	srl	$len,$len,4		# number of complete blocks
	bnez	$len,.Labort
	addi	$sp,$sp,-4*12
	sw	$ra, 4*11($sp)
	sw	$s0, 4*10($sp)
...
.Laligned_inp:
	...
	mulhu	$t4,$rs3,$h1		# h1*s3
	mul	$t3,$rs3,$h1

	add	$h4,$h4,$padbit
	add	$h4,$h4,$d3

	mulhu	$t6,$rs2,$h2		# h2*s2
	mul	$a3,$rs2,$h2
	 add	$d0,$d0,$t3
	 add	$d1,$d1,$t4
	 sltu	$t3,$d0,$t3
	...
	mulhu	$t4,$r0,$h1		# h1*r0
	mul	$t3,$r0,$h1
	 add	$d1,$d1,$a3
	 sltu	$a3,$d1,$a3
	 add	$d2,$d2,$a3

	mulhu	$a3,$rs3,$h2		# h2*s3
	mul	$t6,$rs3,$h2
	 add	$d1,$d1,$t3
	 add	$d2,$d2,$t4
	 sltu	$t3,$d1,$t3
	...
	mulhu	$d3,$r2,$h0		# h0*r2
	mul	$t3,$r2,$h0
	 add	$d1,$d1,$a3
	 sltu	$a3,$d1,$a3
	 add	$d2,$d2,$a3

	mulhu	$t6,$r1,$h1		# h1*r1
	mul	$a3,$r1,$h1
	 add	$d2,$d2,$t3
	 sltu	$t3,$d2,$t3
	 add	$d3,$d3,$t3

	mulhu	$t4,$r0,$h2		# h2*r0
	mul	$t3,$r0,$h2
	 add	$d2,$d2,$a3
	 add	$d3,$d3,$t6
	 sltu	$a3,$d2,$a3
	...
	mulhu	$t6,$r3,$h0		# h0*r3
	mul	$a3,$r3,$h0
	 add	$d2,$d2,$t3
	 sltu	$t3,$d2,$t3
	 add	$d3,$d3,$t3

	mulhu	$t4,$r2,$h1		# h1*r2
	mul	$t3,$r2,$h1
	 add	$d3,$d3,$a3
	 sltu	$a3,$d3,$a3
	 add	$t6,$t6,$a3

	mulhu	$a3,$r0,$h3		# h3*r0
	mul	$h3,$r0,$h3
	 add	$d3,$d3,$t3
	 add	$t6,$t6,$t4
	 sltu	$t3,$d3,$t3
