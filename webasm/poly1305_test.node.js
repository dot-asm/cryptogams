...
const buf = fs.readFileSync('poly1305_global.wasm');
...
(async function main() {
    ...
    const mod = await WebAssembly.instantiate(wasm, {env: {memory : mem}});

    tests.forEach((vector, index) => {
        let i;

        for (i = 0; i < vector.key.buffer.byteLength; i++)
            raw[i] = vector.key[i];

        for (i = 0; i < vector.input.buffer.byteLength; i++)
            raw[32+i] = vector.input[i];

        // first argument is ignored, which is why it's -1
        mod.exports.poly1305_init(-1, 0);
        mod.exports.poly1305_blocks(-1, 32, vector.input.buffer.byteLength);
        mod.exports.poly1305_emit(-1, 0, 16);

        for (i = 0; i < vector.expected.buffer.byteLength; i++) {
            if (raw[i] !== vector.expected[i])
                console.log("test #", index, "mismatch @", i,
    ...
    for (i = 0; i < 100000; i++)
        mod.exports.poly1305_blocks(-1, 0, blksz);
    let elapsed = process.hrtime(start);

    console.log(i * blksz / (elapsed[0] + elapsed[1] * 1.0e-9));
